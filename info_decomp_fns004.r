...
dist_l_filtering_2p <- function(dist_l) {
  ...
  ### get rid of empty streets, where everyone folded
...
build_poker_distribution_2p <- function(dist_l, indices=NA, ordered=F) {
  ...
  dist_2p_fa <- array(0, dim=c(3,3,3,3), dimnames=c("extr1", "intr1", "extr2", "intr2"))
  dist_2p_sa <- array(0, dim=c(3,3,3,3), dimnames=c("extr1", "intr1", "extr2", "intr2"))
    dist_1p_b[,,] <- 0
    dist_2p_ff[,,,] <- 0
    dist_2p_sf[,,,] <- 0
    dist_2p_ss[,,,] <- 0
...
bootstrap_statistics_2p <- function(dist_l, reps) {
  ### filter
...
bootstrap_distributions_2p <- function(dist_l, reps, ordered=F) {
  ...
  for (e in 1:reps) {
    print(e)
    distributions[[e]] <- build_poker_distribution_2p(dist_l, ordered=ordered)
  }
  return(distributions)
}

bootstrap_distributions_to_file <- function(input_file, output_file, reps, ordered=NA) {
Sys.time()
distributions <- bootstrap_distributions_2p(read.csv(input_file), reps, ordered=ordered)
save(distributions, file=output_file)
Sys.time()
}

censor_dist <- function(distribution) {
  #return(distribution[2:3,1:3,2:3,1:3])
  #return(distribution[1:3,2:3,1:3,2:3])
  return(distribution)
}

get_statistics_2p_from_file <- function(input_file, specific=NA) {
  load(file=input_file)
  statistics <- abind(get_statistics_2p(censor_dist(distributions[[1]]$sa), specific=specific), get_statistics_2p(censor_dist(distributions[[1]]$fa), specific=specific), get_statistics_2p( censor_dist(distributions[[1]]$sf), specific=specific), get_statistics_2p(censor_dist(distributions[[1]]$ff), specific=specific), rev.along=0, new.names=list(NULL, c("sa", "fa", "sf", "ff")))
  if(length(distributions)<=1) {
    ...
    for (j in 2:length(distributions)) {
      ...
      statistics <- abind(statistics, abind(get_statistics_2p(censor_dist(distributions[[j]]$sa), specific=specific), get_statistics_2p(censor_dist(distributions[[j]]$fa), specific=specific), get_statistics_2p( censor_dist(distributions[[j]]$sf), specific=specific), get_statistics_2p(censor_dist(distributions[[j]]$ff), specific=specific), rev.along=0), along=along)
    }
  }
...
stat_long_spec_normalized_patch <- function(stat_long) {
