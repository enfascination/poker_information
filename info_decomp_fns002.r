...
dist_l_filtering_2p <- function(dist_l) {
...
build_poker_distribution_2p <- function(dist_l, indices=NA) {
...
get_statistics_2p_across_settings <- function(dist_2p_ss, dist_2p_sf, dist_2p_ff, specific=NA) {
  return(abind(get_statistics_2p(dist_2p_ss, specific=specific), get_statistics_2p( dist_2p_sf, specific=specific), get_statistics_2p(dist_2p_ff, specific=specific), along=3))
}
get_statistics_2p <- function(dist_2p, specific=NA) {
  statistics <- array(0,c(12), dimnames=list(c("ui1", "ue2", "syn","rdn", "mi12", "mi34", "mi13", "ti", "ti2", "h12","h34","h13")))
  dist_2p_d <- dist_2p/sum(dist_2p)
  3vdist <- apply(dist_2p_d, c(1,2,3), sum)
  if (is.na(specific)) {
    statistics[c("rdn")] <- redundancy_3ddist( 3vdist )
    statistics[c("ui1","ue2")] <- c(uniqueinfo_3ddist(3vdist, 2) , uniqueinfo_3ddist(3vdist, 3) )
    statistics[c("syn")] <- synergy_3ddist( 3vdist )
    ...
    statistics[c("ti")] <- totalinfo_3ddist( 3vdist )
    ...
    statistics[c("rdn")] <- specific_redundancy_3ddist( 3vdist , specific)
    statistics[c("ui1","ue2")] <- c(specific_uniqueinfo_3ddist(3vdist, specific, 2) , specific_uniqueinfo_3ddist(3vdist, specific, 3) )
    statistics[c("syn")] <- specific_synergy_3ddist( 3vdist , specific)
    ...
    statistics[c("ti")] <- specinfo_3ddist( 3vdist , specific)
    statistics[c("ti2")] <- statistics[c("ui1")] + statistics[c("ue2")] + statistics[c("syn")] + statistics[c("rdn")]
    statistics[c("h12","h34","h13")] <- c(entropy_adist(apply(dist_2p_d, c(1,2),sum)[specific,]), entropy_adist(apply(dist_2p_d, c(3,4),sum)[specific,]), entropy_adist(apply(dist_2p_d, c(1,3),sum)[specific,]))
    }
  print(statistics[])
  return(statistics)
}
get_statistics_2p_deprecated <- function(dist_2p_ss, dist_2p_sf, dist_2p_ff, specific=NA) {
...
bootstrap_statistics_2p <- function(dist_l, reps, specific=NA) {
  ### filter
  dist_l <- dist_l_filtering_2p( dist_l ) 
  
  statistics <- array(0,c(reps,12,3), dimnames=list(NULL,c("ui1", "ue2", "syn","rdn", "mi12", "mi34", "mi13", "ti", "ti2", "h12","h34","h13"),c("ss","sf","ff")))
  for (e in 1:reps) {
    print(e)
    dist_2p <- build_poker_distribution_2p(dist_l)
    #entropies[e,] <- c(entropy_adist(dist_1p_b[1,,]/sum(dist_1p_b[1,,])), entropy_adist(dist_1p_b[2,,]/sum(dist_1p_b[2,,])))
    #entropies[e,] <- c(entropy_adist(dist_1p_b[1,,]), entropy_adist(dist_1p_b[2,,]), entropy_adist(dist_2p_ff), entropy_adist(dist_2p_sf), entropy_adist(dist_2p_ss))
    statistics[e,,] <- get_statistics_2p(dist_2p$ss, dist_2p$sf, dist_2p$ff, specific )
...
bootstrap_distributions_2p <- function(dist_l, reps) {
  dist_l <- dist_l_filtering_2p( dist_l ) ### filter
  distributions <- list()
  for (e in 1:reps) {
    print(e)
    distributions[[e]] <- build_poker_distribution_2p(dist_l)
  }
  return(distributions)
}

bootstrap_distributions_to_file <- function(input_file, reps, output_file) {
Sys.time()
distributions <- bootstrap_distributions_2p(read.csv(input_file), reps)
save(distributions, file=output_file)
Sys.time()
}
get_statistics_2p_from_file <- function(input_file, specific=NA) {
  load(file=input_file)
  return(get_statistics_2p_across_settings(distributions$ss, dist_2p_ABS_1000$sf, dist_2p_ABS_1000$ff, specific=specific ))
}
...
build_stat_long_2p <- function(statistics) {
  ...
  for (i in 1:max(stat_long$rep)) {
    for (j in c("ss", "sf", "ff")) {
    #for (j in c( "sf", "ff")) {
      ti <- sum(stat_long[(stat_long$rep==i) & (stat_long$setting==j) & (stat_long$q %in% c("ui1", "ue2", "syn", "rdn")),"val"])
      ...
      h13 <- sum(stat_long[(stat_long$rep==i) & (stat_long$setting==j) & (stat_long$q == "h13"), "val" ])
      stat_long <- rbind(stat_long, data.frame(rep=i, val=ti, q="ti3", setting=j))
      stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "ui1"),"val"]/ti, q="ui1n", setting=j))
      stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "ue2"),"val"]/ti, q="ue2n", setting=j))
      ...
      stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "rdn"),"val"]/ti, q="q5n", setting=j))
      ...
      stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "mi12"),"val"]/h12, q="mi12n", setting=j))
      stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "mi34"),"val"]/h34, q="mi34n", setting=j))
      stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "mi13"),"val"]/h13, q="mi13n", setting=j))
    }
  }
