...
ci_width <- 0.95
ci_lci <- (1-ci_width)/2
ci_hci <- 1-(1-ci_width)/2

# inspried by mean_se in ggplot2
mean_ci <- function(y, cis=c(ci_lci, ci_hci)) {
    stats <- c(mean(y), quantile(y, c(0.025, 0.975)))
    names(stats) <- c("y", "ymin", "ymax")
    return(stats)
}
...
ps <- ggplot(stat_long_2p_fig4, aes(factor(blind), val, group=setting, color=setting) ) + geom_point(size=0.5, position = position_jitter(width = 0.25, height=0)) + stat_summary(fun.data=mean_ci, geom="errorbar", width= 0.5) + facet_grid( q ~ Dataset  , scales='free_y') + scale_x_discrete("Betting level ($)")+ scale_y_continuous("Bits")+  scale_color_brewer("Focal player", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position = c(.68,.92), legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm")) +  guides(colour = guide_legend(override.aes = list(size=2))); ps
...
ps <- ggplot(stat_long_st_pub , aes(factor(blind), val, group=setting, color=setting) ) + geom_point(size=0.25, position = position_jitter(width = 0.25, height=0)) + stat_summary(fun.data=mean_ci, geom="errorbar", width= 0.5) + facet_grid( q ~ street   , scales='free_y') + scale_x_discrete("Betting level ($)")+ scale_y_continuous("Bits")+  scale_color_brewer("Focal player", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position = c(.91,.90), legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm")) +  guides(colour = guide_legend(override.aes = list(size=2))); ps
