...
rownames(order_play) <- c("game", "IDs", "street", "seats", "bet", "order_seat", "order_play", "button")
...
order_play_idx <- 1
last_hand <- 0
bb <- as.numeric(barelyknower[1,"blnd"])/2
for (i in 1:nrow(order_play)) {
...
### fix some errors introduced in earlier processing stages
