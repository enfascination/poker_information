...
### START SAMPLING
blinds <- c("1000","0600","0400","0200", "0100", "0050", "0025")
dist_l_all <- list()
...
print("main decomposition: 25 cents")
    ...
    in_file <- paste0(path_poker, "distrPS", blinds[7], ".csv")
...
dist_l  <- data.table()
for(ib in 1:(length(blinds)-1)) {
    print(blinds[ib])
    in_file <- paste0(path_poker, "distrPS", blinds[ib], ".csv")
	tmp <- fread(in_file)
    ...
    print(paste0("number of hands: ", dist_l[,length(unique(hand))]))
    print(paste0("rows: ", nrow(dist_l)))
}
out_file <- paste0(path_poker_data, "poker_distributions_2p_PS_large.Rdata")
bootstrap_distributions_to_file(data.matrix(dist_l), output_file=out_file, reps, ordered=tagopts$ordered, extrAsWagerOrAction=tagopts$extr)
blinds <- c("small", "large")
###    RELOAD THE SAMPLES THEN CONSOLIDATE
stat_long_2p_list <- rep(list(0),length(blinds))
for(ib in 1:length(blinds)) {
    in_file <- paste0(path_poker_data, "poker_distributions_2p_PS_", blinds[ib], ".Rdata")
        ppsite <- "PS"; ppblind <- blinds[ib];
...
n_channels <- 3
stat_long_2p_spec_list <- rep(list(0),length(blinds) * n_channels)
il <- 1
for(ib in 1:length(blinds)) {
    print(blinds[ib])
...
if( tagopts$onestreet == FALSE  ) { ### (skip if this is a one-street robustness analysis)
    ...
    for(ib in 1:length(blinds)) {
        ...
        in_file <- paste0(path_poker, "distrPS", blinds[ib], ".csv")
        if (tagopts$riskmemorycrash) {
            dist_l <- dist_l_all[[ib]]
        } else {
            dist_l <- fread(in_file)
        }
        for(st in 1:n_streets) {
            out_file <- paste0(path_poker_data, "poker_distributions_2p_PS_", blinds[ib], "_st", st, ".Rdata")
    ...
    stat_long_2p_st_list <- rep(list(0),length(blinds) * n_streets)
    il <- 1
    for(ib in 1:length(blinds)) {
        for(st in 1:n_streets) {
            in_file <- paste0(path_poker_data, "poker_distributions_2p_PS_", blinds[ib], "_st", st, ".Rdata")
            ppsite <- "PS"; ppblind <- blinds[ib];
