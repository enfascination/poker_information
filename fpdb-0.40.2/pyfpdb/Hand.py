...
#I think the antes should be common money, don't have enough hand history to check
    ...
    ### Seth created this for himself
    def get_actions_code(self, player, street):
        """ Returns an integer for the actions of the given player and the given street
            Antes, blinds are 0
            if the player folded right away, 1
            if the player folded after some haggling, 1.5
            if the player stayed in easily 2
            if the player stayed in after some haggling 2.5
            if the player is out 3

            OR
            if the player started by folding 1
            if the player started by calling or checking 2
            if the player started by betting or raising 3
            if the player ended by folding after some haggling add .1
            if the player ended by calling or checking after some haggling add .2
            if the player ended by betting or raising after some haggling add .3
            if the player is out 4
            if the player antes or blinds 0
        ...
        if street == 'BLINDSANTES': 
          list.append('A') ### A for ante/blind
          return(0)
        else:
          for action in actions:
              if player in action:
                  if action[1] == 'folds':
                      ...
                      list.append('R')
        if list == []: list.append('O') ### O for "out" of the hand
    ...
    def get_actions_short(self, player, street):
        """ Returns a string with shortcuts for the actions of the given player and the given street
            F ... fold, X ... Check, B ...Bet, C ... Call, R ... Raise
        """
        ### Seth added some things here
        actions = self.actions[street]
        list = []
        if street == 'BLINDSANTES': list.append('A') ### A for ante/blind
        else:
          for action in actions:
              if player in action:
                  if action[1] == 'folds':
