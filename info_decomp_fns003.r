...
bootstrap_distributions_to_file <- function(input_file, reps, output_file, ordered=F) {
Sys.time()
...
get_statistics_2p_from_file <- function(input_file, specific=NA) {
  ...
  statistics <- abind(get_statistics_2p(censor_dist(distributions[[1]]$ss), specific=specific), get_statistics_2p( censor_dist(distributions[[1]]$sf), specific=specific), get_statistics_2p(censor_dist(distributions[[1]]$ff), specific=specific), rev.along=0, new.names=list(NULL, c("ss", "sf", "ff")))
  if(length(distributions)<=1) {
    ...
    for (j in 2:length(distributions)) {
      ...
      statistics <- abind(statistics, abind(get_statistics_2p(censor_dist(distributions[[j]]$ss), specific=specific), get_statistics_2p( censor_dist(distributions[[j]]$sf), specific=specific), get_statistics_2p(censor_dist(distributions[[j]]$ff), specific=specific), rev.along=0), along=along)
    }
  }
...
build_stat_long_2p <- function(statistics) {
  ...
  stat_long <- rbind(stat_long, stat_long_normalized(stat_long[stat_long$setting=="ff",], "ff"), stat_long_normalized(stat_long[stat_long$setting=="sf",], "sf"), stat_long_normalized(stat_long[stat_long$setting=="ss",], "ss"))
