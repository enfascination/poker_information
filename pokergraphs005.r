...
stat_long$q <- as.character(stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q1", "ui1", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q2", "ue2", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q4", "syn", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q5", "rdn", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q1n", "ui1n", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q2n", "ue2n", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q4n", "synn", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q5n", "rdnn", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q3i1", "mi12", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q3i2", "mi34", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q3i1n", "mi12n", stat_long$q)
stat_long$q <- ifelse(stat_long$q == "q3i2n", "mi34n", stat_long$q)
...
ggplot(subset(stat_long, site == "PS" & q %in% c("q3i1", "q3i2", "mi13") & setting %ni% c("Shark v Shark")), aes(factor(blind), val, group=setting, color=setting) ) + geom_point(position = position_jitter(width = .25)) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + facet_wrap( ~ q, ncol=2) + scale_x_discrete("Measure")+ scale_y_continuous("Percentage of total information")
