...
dist_l <- as.matrix(dist_l[dist_l[,"seats"] ==2,c("street", "skil1", "extr1", "intr1", "skil2", "extr2", "intr2")])
...
### changing this, say by sampling indices with replacement, will let me bootstrap distributiosn and measures over them
if (testing) {
  statistics <- array(0,c(1,12,3), dimnames=list(NULL,c("H1", "H2", "q1", "q2", "q3_i1", "q3_i2","q4","q5", "ti", "mi13", "h12", "h34"),c("ss","sf","ff")))
  indices <- 1:nrow(dist_l)
  e <- 1
  scales <- c(2,4)
  dist_1p <- array(0, dim=c(2,3,3), dimnames=c("skill", "extr", "intr"))
  ...
  for (i in indices) {
    ...
    if (2 %in% scales) {
      dist_1p[idxs1] <- dist_1p[idxs1] + 1 
      dist_1p[idxs2] <- dist_1p[idxs2] + 1 
    }
    if (4 %in% scales) {
  ...
  dist_1p_s_d <- dist_1p[2,,]/sum(dist_1p[2,,])
  dist_1p_f_d <- dist_1p[1,,]/sum(dist_1p[1,,])
  #dist_2p_ss_d[c(2,3),,c(2,3),]
  #dist_2p_sf_d[c(2,3),,c(2,3),]
  #dist_2p_ff_d[c(2,3),,c(2,3),]
  dist_1p_s_d
  dist_1p_f_d
  #entropy_adist( dist_1p_f_d)
  #entropy_adist( dist_1p_s_d)
  ss_3vdist <- apply(dist_2p_ss_d, c(1,2,3), sum)
  sf_3vdist <- apply(dist_2p_sf_d, c(1,2,3), sum)
  ff_3vdist <- apply(dist_2p_ff_d, c(1,2,3), sum)
  statistics[e,c("H1","H2"),1] <- c( entropy_adist( dist_1p_s_d), entropy_adist( dist_1p_s_d))
  statistics[e,c("H1","H2"),2] <- c( entropy_adist( dist_1p_s_d), entropy_adist( dist_1p_f_d))
  statistics[e,c("H1","H2"),3] <- c( entropy_adist( dist_1p_f_d), entropy_adist( dist_1p_f_d))
...
file_output <- "~/projecto/research_projects/poker_information/diststatistics006.csv"
