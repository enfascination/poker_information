...
max_fig1_a <- 1.75
max_fig1_c <- 1.0
stat_long_pub <- subset(stat_long_2p,  q %in% c("h1", "ti", "ti_h1") & setting %ni% c("Shark v Shark"))
stat_long_pub$q <- factor(as.character(stat_long_pub$q), levels=c("h1", "ti", "ti_h1"), labels=c("H(W1)", "I(W1;P1,W2)", "I(W1;P1,W2)/H(W1)"))
p <- ggplot(subset(stat_long_pub,  q %in% c("H(W1)", "I(W1;P1,W2)") ), aes(factor(blind), val, group=setting, color=setting) ) + geom_point(position = position_jitter(width = .25), size=0.5) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + facet_wrap( ~ q, ncol=4) + scale_x_discrete("Blind")+ scale_y_continuous("Bits", limits=c(0,max_fig1_a))+  scale_color_brewer("Match", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position="none") #; p
q <- ggplot(subset(stat_long_pub,  q %in% c("I(W1;P1,W2)/H(W1)") ), aes(factor(blind), val, group=setting, color=setting) ) + geom_point(position = position_jitter(width = .25), size=0.5) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + scale_x_discrete("Blind")+ scale_y_continuous("% total info", limits=c(0,max_fig1_c ))+  scale_color_brewer("Match", type="qual", palette=2) + labs(title="I(W1;P1,W2)/H(W1)") + theme_few() + theme( aspect.ratio=0.6, legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm"), plot.margin=unit(c(1,0,0.75,0), "cm"), plot.title=element_text(size=10)) +  guides(colour = guide_legend(override.aes = list(size=2)))#; q
...
max_fig2_a <- 0.75
stat_long_pub <- subset(stat_long_2p,  q %in% c("ui1", "ue2", "rdn", "syn") & setting %ni% c("Shark v Shark"))
...
p <- ggplot(stat_long_pub, aes(factor(blind), val, group=setting, color=setting) ) + geom_point(position = position_jitter(width = .25), size=0.5) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + facet_wrap( ~ q, ncol=4) + scale_x_discrete("Blind")+ scale_y_continuous("Bits", limits=c(0,max_fig2_a))+  scale_color_brewer("Match", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position = c(.93,.75), legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm")) +  guides(colour = guide_legend(override.aes = list(size=2))); p
### save figure two
...
### figure Three
stat_long_spec_pub <- subset(stat_long_2p_spec, q %in% c("ui1", "ue2", "rdn", "syn") & setting %ni% c("Shark v Shark"))
...
stat_long_spec_pub$specific <- factor(as.character(stat_long_spec_pub$specific), levels=c(1,2,3), labels=c("No wager", "Small wager", "Big wager"))
ps <- ggplot(stat_long_spec_pub , aes(factor(blind), val, group=setting, color=setting) ) + geom_point(size=0.5, position = position_jitter(width = 0.25, height=0)) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + facet_grid( specific ~ q ) + scale_x_discrete("Blind")+ scale_y_continuous("Bits")+  scale_color_brewer("Match", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position = c(.91,.90), legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm")) +  guides(colour = guide_legend(override.aes = list(size=2))); ps
...
load(file=paste("~/projecto/research_projects/poker_information/poker_graph_ready_data_", "032_100_noante_showdown", ".Rdata", sep=''))
...
stat_long_2p_fig4 <- subset(stat_long_2p_fig4, q %in% c("ui1", "ue2", "rdn", "syn") & setting %ni% c("Shark v Shark"))
...
#stat_long_2p_fig4$specific <- factor(as.character(stat_long_2p_fig4$specific), levels=c(1,2,3), labels=c("No wager", "Small wager", "Big wager"))
ps <- ggplot(stat_long_2p_fig4, aes(factor(blind), val, group=setting, color=setting) ) + geom_point(size=0.5, position = position_jitter(width = 0.25, height=0)) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + facet_grid( q ~ Dataset  , scales='free_y') + scale_x_discrete("Blind")+ scale_y_continuous("Bits")+  scale_color_brewer("Match", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position = c(.68,.92), legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm")) +  guides(colour = guide_legend(override.aes = list(size=2))); ps
...
ps <- 
  ggplot(subset(stat_long_2p_fig5, (Dataset != "Full") & (blind!=0.25)), aes(factor(q), (`val.Shark v Fish`-`val.Fish v Fish`)) ) + geom_point(size=0.5, position = position_jitter(width = 0.25, height=0)) + stat_summary(fun.y=mean, fun.ymin=function(y) quantile(y, c(0.025)), fun.ymax=function(y) quantile(y, c(0.975)), geom="errorbar", width= 0.5) + facet_grid( Dataset ~ .) 
+ scale_x_discrete("Blind")+ scale_y_continuous("Bits")+  scale_color_brewer("Match", type="qual", palette=2) + theme_few() + theme(aspect.ratio=0.6, legend.position = c(.68,.92), legend.background = element_rect(colour = 'gray',  size = 0.2), legend.key.size = unit(0.3, "cm")) +  guides(colour = guide_legend(override.aes = list(size=2))); ps
