...
### changing this, say by sampling indices with replacement, will let me bootstrap distributiosn and measures over them
  ...
  scales <- c(2,4)
  reps <- 200
  statistics <- array(0,c(reps,9,3), dimnames=list(NULL,c("h1f", "h2p", "h3s", "hfps", "u2", "u3", "rdn", "syn","ti"),c("sss", "fss","ffs")))
  ...
  for (e in 1:reps) {
    ...
    for (i in indices) {
      ...
      if (3 %in% scales) {
        idxs12 <- rbind(dist_l[i,c("extr1", "extr2", "extr3")]+1)
...
file_output <- "~/projecto/research_projects/poker_information/3pdiststatistics007.csv"
