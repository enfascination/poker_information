...
build_poker_distribution_2p <- function(dist_l, reps) {
...
### changing this, say by sampling indices with replacement, will let me bootstrap distributiosn and measures over them
  ...
  statistics <- array(0,c(reps,11,3), dimnames=list(NULL,c("ui1", "ue2", "q3_i1", "q3_i2","syn","rdn", "ti", "mi13", "ti2", "h12","h34"),c("ss","sf","ff")))
  scales <- c(2,4)
  scales <- c(4)
...
for (i in 1:max(stat_long$rep)) {
  ...
  #for (j in c( "sf", "ff")) {
    ...
    stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "ui1"),"val"]/ti, q="q1n", setting=j))
    stat_long <- rbind(stat_long, data.frame(rep=i, val=stat_long[(stat_long$rep ==i) & (stat_long$setting==j) & (stat_long$q == "ue2"),"val"]/ti, q="q2n", setting=j))
